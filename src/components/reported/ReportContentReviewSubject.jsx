...
function ReportContentReviewSubject({ report }) {
  ...
  const [updateReportContentReviewSubject] =
    useUpdateReportContentReviewSubjectMutation();
  const [deleteReportContentReviewSubject] =
    useDeleteReportContentReviewSubjectMutation();
  const onUpdate = () => {
    const formData = new FormData();
    formData.append("message", newMessage);
    updateReportContentReviewSubject({
      reviewSubjectId: report.reviewSubject.id,
      reportContentReviewSubjectId: report.id,
      body: formData,
    }).then((response) => {
      if (!response.error) {
        setOnEdit(false);
  ...
  const onDelete = () => {
    deleteReportContentReviewSubject({
      reviewSubjectId: report.reviewSubject.id,
      reportContentReviewSubjectId: report.id,
    }).then((response) => {
      if (!response.error) {
    ...
    >
      ...
      >
        <Box
          width={"100%"}
          height={"100%"}
          dangerouslySetInnerHTML={{ __html: report?.reviewSubject.review }}
...
export default ReportContentReviewSubject;
