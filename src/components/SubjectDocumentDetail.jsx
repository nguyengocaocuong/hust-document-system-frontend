...
function SubjectDocumentDetail() {
  ...
  const handleSelectLanguage = async (value) => {
    ...
    setLanguage({ value, loading: true });
    fetch(
      `https://hust-document-system-backend-vxw4qk34wa-as.a.run.app/api/v1/users/subjects/subjectDocument/${id}/translate?targetLanguage=${value}&${location.search}`,
      {
        method: "GET",
        headers: {
          "X-HUST-DOCUMENT-KEY": user.token,
  ...
  useEffect(() => {
    if (isSuccess && instance)
      instance.UI.TabManager.addTab(
        `https://hust-document-system-backend-vxw4qk34wa-as.a.run.app/api/v1/users/subjects/subjectDocument/${id}/readFile?${location.search}`,
        {
  ...
  const onShowAnnotate = (annotateId) => {
    fetch(
      `https://hust-document-system-backend-vxw4qk34wa-as.a.run.app/api/v1/users/subjects/subjectDocument/${id}/answerSubjectDocument/${annotateId}/readFile`,
      {
        method: "GET",
        headers: {
          "X-HUST-DOCUMENT-KEY": user.token,
